...
public class AppInitServlet extends HttpServlet {
  ...
  // Servlet life cycle 3가지는 service init destory 서블릿 컨테이너(WAS)가 관리한다.
  ...
  public void init() throws ServletException {
    ...
    try {
      ...
      //        
      //        서블릿이나 필터 리스너를 초기화할때는 init를 사용해라.!
      //       생성자에서 getServletContext()를 호출하면 오류 발생!
      //       왜? 
      //        - ServletConfig 객체를 통해 꺼낼  수가  있는데,
      //       
      //        -ServletConfig 는 아직 주입되지 않은 상태이다.
      //    
      //       ServletConfig  객체가 언제 주입되는가?
      //        -생성자 다음에 호출되는 init()가 호출될 때 ServletConfig객체가 주입된다.

      //       그래서 생성자 다음에 호출되는 init() 에서 getServletContext()를 호출해야한다.
      ServletContext ctx = this.getServletContext();
      // ServletContext 서블릿 컨테이너를 부를수 있게 도와주는 인터페이스고 
      //System.out.println(ctx); // org.apache.catalina.core.ApplicationContextFacade@58015e56 
      // ApplicationContextFacade 생명주기를 관리하는 핵심적인 부분.
